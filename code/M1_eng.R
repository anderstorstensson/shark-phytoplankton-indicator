...
data_all <- read.csv("../data/processed_data/shark_data.csv", sep=';', dec=',') #"example.csv" - the name of your file
...
jpeg (paste0("../plots/", title))
 ...
 title <- paste("../data/processed_data/", name_taxa, "_score_", "test_",min(eqr_test$year), "_", max(eqr_test$year), "_", "ref_", min(eqr_ref$year), "_", max(eqr_ref$year), ".csv", sep = "")
 
 write.table(score, file = title, quote = TRUE, sep = ";",
             eol = "\n", na = "NA", dec = ",", row.names = T,col.names = NA)

 
 title1 <- paste("../data/processed_data/", name_taxa, "_", "test_",min(eqr_test$year), "_", max(eqr_test$year), "_", "ref_", min(eqr_ref$year), "_", max(eqr_ref$year), ".csv", sep = "")
 title2 <- paste("../data/processed_data/", name_taxa, "_", "test_",min(eqr_ref$year), "_", max(eqr_ref$year), "_", "ref_", min(eqr_ref$year), "_", max(eqr_ref$year), ".csv", sep = "")
