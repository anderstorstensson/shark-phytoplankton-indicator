...
params:
  shark_data_file: "Släggö.txt" # Your shark file
...
selected_data = shark_data %>%
  filter(parameter == params$parameter) %>%
  filter(taxon_phylum %in% c("Bacillariophyta", "Cyanobacteria") |
           scientific_name == "Mesodinium rubrum" |
           taxon_phylum == "Miozoa" & trophic_type_code %in% c("AU","MX")) %>%
  filter(sample_min_depth_m == 0)
